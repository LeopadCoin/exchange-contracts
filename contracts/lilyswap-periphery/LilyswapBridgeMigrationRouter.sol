...
contract LilyswapBridgeMigrationRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'LilyswapBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'LilyswapBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "LilyswapBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "LilyswapBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "LilyswapBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "LilyswapBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        ILilyswapERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = ILilyswapPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = LilyswapLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = LilyswapLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = ILilyswapPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = ILilyswapPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "LilyswapBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "LilyswapBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "LilyswapBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            ILilyswapPair(pairA).token0() == ILilyswapPair(pairB).token0() ||
            ILilyswapPair(pairA).token0() == ILilyswapPair(pairB).token1() ||
            ILilyswapPair(pairA).token1() == ILilyswapPair(pairB).token0() ||
            ILilyswapPair(pairA).token1() == ILilyswapPair(pairB).token1(),
            "LilyswapBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "LilyswapBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "LilyswapBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        ILilyswapPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = ILilyswapPair(liquidityPairFrom).token0();
        if (
            ILilyswapPair(liquidityPairFrom).token0() == ILilyswapPair(liquidityPairTo).token0() ||
            ILilyswapPair(liquidityPairFrom).token0() == ILilyswapPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = ILilyswapPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "LilyswapBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == ILilyswapPair(liquidityPairTo).token0() ||
            newTokenAddress == ILilyswapPair(liquidityPairTo).token1(),
            "LilyswapBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != ILilyswapPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (ILilyswapPair(liquidityPairFrom).token0() != ILilyswapPair(liquidityPairTo).token0() &&
            ILilyswapPair(liquidityPairFrom).token1() != ILilyswapPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            ILilyswapPair(liquidityPairTo).token0(), ILilyswapPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(ILilyswapPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(ILilyswapPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = ILilyswapPair(pairAddress).getReserves();
        uint totalSupply = ILilyswapPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "LilyswapBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "LilyswapBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = ILilyswapPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = LilyswapLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
